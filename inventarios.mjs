...
import {carpetaDestinoDesperdicios} from './datosUsuario.mjs'
import {carpetaDestinoConsumos} from './datosUsuario.mjs'
import {carpetaDestinoDonaciones} from './datosUsuario.mjs'
import {carpetaDestinoCortesia} from './datosUsuario.mjs'
...
const anioFinal = fechaFinal.slice(4);      
  ...
  const seleccionGrupoLocales = (grupoLocalesSeleccionado) => {
    switch (grupoLocalesSeleccionado) {
      ...
      default:
        console.log('Nombre de grupo no válido. Elige entre localesTodos, localesTodosPropios, localesTeaConnection, localesGreenEat, localesCasaSaenz, localesAliadosExternos o localesSeleccionPersonalizada.');
        rl.close();
    }
  };
...
const reporteBodegaDesperdicio = async(letraInicioSelect, vecesArrowDown, local) => {
    ...
    try {
        ...
        console.log(local + ' Generando Reporte - Desperdicios') // click actualizar datos. ¿puede reconocer cuando termina de cargar datos?
        await waitForSelectorReintentos(page, 'th[ng-click="sortConsolidadosRows(1)"]' )  
        await waitForSelectorReintentos(page, 'a[id="linkDl-5-1"]' )  
        await page.click('a[id="linkDl-5-1"]')
        console.log(local + ' Descarga ejecutada - Desperdicios')
   ...
   /*      console.error('Aviso de error en bloque try. Reintento de descarga', error)  */
    ...
    let tipoReporte = "Desperdicio"
    moverArchivoYCambioNombre(tipoReporte, carpetaDestinoDesperdicios)

}

const reporteBodegaConsumos = async(letraInicioSelect, vecesArrowDown, local) => {
            // Seleccion parametro Bodega Desperdicio
            try {
            await waitForSelectorReintentos(page, 'select[ng-change="cambioBodegaInvFinal()"]' )   
            await page.click('select[ng-change="cambioBodegaInvFinal()"]')
            await page.keyboard.press(`${letraInicioSelect}`);
            ...
            await page.waitForTimeout(2000)
            await page.click('button[ng-click="generaConsolidados()"]')
            console.log(local + ' Generando Reporte - Consumos') // click actualizar datos. ¿puede reconocer cuando termina de cargar datos?
            await waitForSelectorReintentos(page, 'th[ng-click="sortConsolidadosRows(1)"]' )   
            await page.waitForTimeout(5000);
            await waitForSelectorReintentos(page, 'a[id="linkDl-5-1"]' )   
            await page.click('a[id="linkDl-5-1"]')
            console.log(local + ' Descarga ejecutada - Consumos')
            await page.waitForTimeout(20000);
        } catch (error) {
        /*   console.error('Aviso de error en bloque try. Reintento de descarga', error)  */
          throw error;
  ...
  let tipoReporte = "Consumos"
  moverArchivoYCambioNombre(tipoReporte,carpetaDestinoConsumos)
}

const reporteBodegaDonacion = async(letraInicioSelect, vecesArrowDown, local) => {
    // Seleccion parametro Bodega Desperdicio
    ...
    await page.waitForTimeout(2000)
    await page.click('button[ng-click="generaConsolidados()"]')
    console.log(local + ' Generando Reporte - Donaciones') // click actualizar datos. ¿puede reconocer cuando termina de cargar datos?
    await waitForSelectorReintentos(page, 'th[ng-click="sortConsolidadosRows(1)"]')     
    await page.waitForTimeout(5000);

    await waitForSelectorReintentos(page,'a[id="linkDl-5-1"]')   
    await page.click('a[id="linkDl-5-1"]')
    console.log(local + ' Descarga ejecutada - Donaciones')
    await page.waitForTimeout(20000);
} catch (error) {
/*   console.error('Aviso de error en bloque try. Reintento de descarga', error)  */
  throw error;
  ...
  // Clck descargar
    ...
    let tipoReporte = "Donacion"
    moverArchivoYCambioNombre(tipoReporte, carpetaDestinoDonaciones)
}


const reporteBodegaCortesia = async(letraInicioSelect, vecesArrowDown, local) => {
  // Seleccion parametro Bodega Desperdicio
  ...
  await page.waitForTimeout(2000)
  await page.click('button[ng-click="generaConsolidados()"]')
  console.log(local + ' Generando Reporte - Cortesias') // click actualizar datos. ¿puede reconocer cuando termina de cargar datos?
  await waitForSelectorReintentos(page, 'th[ng-click="sortConsolidadosRows(1)"]')     
  await page.waitForTimeout(5000);

  await waitForSelectorReintentos(page,'a[id="linkDl-5-1"]')   
  await page.click('a[id="linkDl-5-1"]')
  console.log(local + ' Descarga ejecutada - Cortesias')
  await page.waitForTimeout(20000);
} catch (error) {
/*   console.error('Aviso de error en bloque try. Reintento de descarga', error)  */
throw error;
...
// Clck descargar
  ...
  let tipoReporte = "Cortesia"
  moverArchivoYCambioNombre(tipoReporte, carpetaDestinoCortesia)
}
...
export let moverArchivoYCambioNombre = async (tipoReporte, carpetaDestino) => {
...
// EJECUCION DEL PROGRAMA
  ...
  (async () => {
    console.log('Proceso Descarga Reportes Inventarios (Desperdicios, Consumos y Donaciones) Inicializado')
    ...
    const funcionesEjecutadas = { // condiciones para evitar repetir descargas de reportes ya ejecutados.
      'ReporteDesperdicio': false,
      'ReporteConsumo': false,
      'ReporteDonacion': false,
      'ReporteCortesia': false,
    };
          ...
          while(!iteracionCompletaExitosa)
            try {
              ...
              if(local.includes('TEAAR')) {
                    if (!funcionesEjecutadas['ReporteDesperdicio']) { // VERIFICA SI LA FUNCION ESTA EJECUTADA PARA NO REPETIRLA EN EL BUCLE DE REINICIO.
                            await reporteBodegaDesperdicio('t', 4, local) // luego condicionar los reportes segun los locales
                            funcionesEjecutadas['ReporteDesperdicio'] = true;
                    }
                    if (!funcionesEjecutadas['ReporteConsumo']) { // VERIFICA SI LA FUNCION ESTA EJECUTADA PARA NO REPETIRLA EN EL BUCLE DE REINICIO.
                      await reporteBodegaConsumos('t', 3, local) // luego condicionar los reportes segun los locales
                      funcionesEjecutadas['ReporteConsumo'] = true;
                    }

  
              }if(local.includes('CSZAR')) {
                    if (!funcionesEjecutadas['ReporteDesperdicio']) { // VERIFICA SI LA FUNCION ESTA EJECUTADA PARA NO REPETIRLA EN EL BUCLE DE REINICIO.
                      await reporteBodegaDesperdicio('t', 3, local) // luego condicionar los reportes segun los locales
                      funcionesEjecutadas['ReporteDesperdicio'] = true;
                    }
                    if (!funcionesEjecutadas['ReporteConsumo']) { // VERIFICA SI LA FUNCION ESTA EJECUTADA PARA NO REPETIRLA EN EL BUCLE DE REINICIO.
                      await reporteBodegaConsumos('t', 4, local) // luego condicionar los reportes segun los locales
                      funcionesEjecutadas['ReporteConsumo'] = true;
                    }

                   /*  if (!funcionesEjecutadas['ReporteCortesia']) { // VERIFICA SI LA FUNCION ESTA EJECUTADA PARA NO REPETIRLA EN EL BUCLE DE REINICIO.
                      await reporteBodegaCortesia('t', 6, local) // luego condicionar los reportes segun los locales
                      funcionesEjecutadas['ReporteCortesia'] = true;
                    } */
                    ...
                    if (!funcionesEjecutadas['ReporteDonacion']) { // VERIFICA SI LA FUNCION ESTA EJECUTADA PARA NO REPETIRLA EN EL BUCLE DE REINICIO.
                      await reporteBodegaDonacion('t', 7, local) // luego condicionar los reportes segun los locales
                      funcionesEjecutadas['ReporteDonacion'] = true;
                    }


                /*   await reporteBodegaDesperdicio('t', 3, local) // luego condicionar los reportes segun los locales
                  await reporteBodegaConsumos('t', 4, local) */
              /*     await reporteBodegaDonacion('t', 7, local) */
              }
              iteracionCompletaExitosa = true
              funcionesEjecutadas['ReporteDesperdicio'] = false;
              funcionesEjecutadas['ReporteConsumo'] = false;
              funcionesEjecutadas['ReporteDonacion'] = false;
              funcionesEjecutadas['ReporteCortesia'] = false;
